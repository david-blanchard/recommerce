...
const CartArticleSet = props => {
    ...
    const booksBulk = cartService.isbnCodes
    ...
    const effect = () => {
        currentCart = cartService.readCart()
        ...
        offerService.getOffersFromBulkCallback(subtotal, booksBulk, (data) => {
            const offers = data.offers

            const discount = offerService.computeDiscount(subtotal, offers)
    ...
    const handleRemove = keyid => {
        cartService.removeFromCart(keyid)
    ...
    return (
        <tr>
            ...
            <td className='text-end'>
                <button onClick={() => {
                    props.onRemove(article.keyid)
                }} className='remove-from-cart-cta btn btn-sm btn-danger'>
